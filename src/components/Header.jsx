
import React from 'react';
import { Link, useLocation } from 'react-router-dom';

export default function Header() {
  const location = useLocation();
  const isCases = location.pathname.includes('cases');

  return (
    <>
      <header id="site-header">
        <div className="header-inner">
            <Link to="/" className="logo" data-i18n="logo">нексиум.дев</Link>

            <nav className="header-nav" aria-label="Main navigation">
                <Link to="/cases" className={`nav-link unbounded ${isCases ? 'is-active' : ''}`}>Портфолио</Link>
                <a href="https://t.me/your_telegram" target="_blank" className="nav-link nav-link--tg unbounded">
                    <svg className="nav-link-icon" fill="currentColor" width="15" height="15" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                        <path d="M22.122 10.040c0.006-0 0.014-0 0.022-0 0.209 0 0.403 0.065 0.562 0.177l-0.003-0.002c0.116 0.101 0.194 0.243 0.213 0.403l0 0.003c0.020 0.122 0.031 0.262 0.031 0.405 0 0.065-0.002 0.129-0.007 0.193l0-0.009c-0.225 2.369-1.201 8.114-1.697 10.766-0.21 1.123-0.623 1.499-1.023 1.535-0.869 0.081-1.529-0.574-2.371-1.126-1.318-0.865-2.063-1.403-3.342-2.246-1.479-0.973-0.52-1.51 0.322-2.384 0.221-0.23 4.052-3.715 4.127-4.031 0.004-0.019 0.006-0.040 0.006-0.062 0-0.078-0.029-0.149-0.076-0.203l0 0c-0.052-0.034-0.117-0.053-0.185-0.053-0.045 0-0.088 0.009-0.128 0.024l0.002-0.001q-0.198 0.045-6.316 4.174c-0.445 0.351-1.007 0.573-1.619 0.599l-0.006 0c-0.867-0.105-1.654-0.298-2.401-0.573l0.074 0.024c-0.938-0.306-1.683-0.467-1.619-0.985q0.051-0.404 1.114-0.827 6.548-2.853 8.733-3.761c1.607-0.853 3.47-1.555 5.429-2.010l0.157-0.031zM15.93 1.025c-8.302 0.020-15.025 6.755-15.025 15.060 0 8.317 6.742 15.060 15.060 15.060s15.060-6.742 15.060-15.060c0-8.305-6.723-15.040-15.023-15.060h-0.002q-0.035-0-0.070 0z"/>
                    </svg>
                    <span data-i18n="header_telegram">Telegram</span>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <line x1="5" y1="12" x2="19" y2="12"></line>
                        <polyline points="12 5 19 12 12 19"></polyline>
                    </svg>
                </a>
            </nav>

            <div className="header-controls">
                <div className="lang-switcher" aria-label="Language switcher">
                    <button className="lang-btn active" type="button" data-lang="ru">RU</button>
                    <button className="lang-btn" type="button" data-lang="en">EN</button>
                </div>

                <button className="burger-btn" id="burger-btn" type="button" aria-label="Open menu" aria-expanded="false" aria-controls="mobile-menu">
                    <span className="burger-bar"></span>
                    <span className="burger-bar"></span>
                    <span className="burger-bar"></span>
                </button>
            </div>
        </div>
      </header>

      <div className="mobile-menu" id="mobile-menu" aria-hidden="true">
        <div className="mobile-menu-inner">
            <nav className="mobile-menu-nav">
                <Link to="/" className="mobile-menu-link">Главная</Link>
                <Link to="/cases" className="mobile-menu-link">Портфолио</Link>
                <a href="https://t.me/your_telegram" target="_blank" className="mobile-menu-link mobile-menu-link--tg">
                    <span data-i18n="header_telegram">Telegram</span>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <line x1="5" y1="12" x2="19" y2="12"></line>
                        <polyline points="12 5 19 12 12 19"></polyline>
                    </svg>
                </a>
            </nav>
            <div className="mobile-menu-footer">
                <span className="mobile-menu-logo" data-i18n="logo">нексиум.дев</span>
            </div>
        </div>
      </div>
      <div className="mobile-menu-backdrop" id="mobile-menu-backdrop"></div>
    </>
  );
}
